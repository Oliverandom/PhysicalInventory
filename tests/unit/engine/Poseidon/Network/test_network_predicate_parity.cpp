...
TEST_CASE("Delayed role assignment starts per-player mission catch-up", "[network][mission]")
{
    ...
    REQUIRE_FALSE(Poseidon::DelayedRoleShouldStartMissionCatchUp(true, prepareRole, prepareRole, true,
                                                                 transferMission, briefing));
    REQUIRE_FALSE(Poseidon::DelayedRoleShouldStartMissionCatchUp(true, briefing, briefing, true, transferMission,
                                                                 briefing));
    REQUIRE_FALSE(Poseidon::DelayedRoleShouldStartMissionCatchUp(true, briefing, transferMission, false,
                                                                 transferMission, briefing));
    REQUIRE_FALSE(Poseidon::DelayedRoleShouldStartMissionCatchUp(true, play, transferMission, true, transferMission,
                                                                 briefing));
}
...
TEST_CASE("Squad logo paths reject unsafe XML path components", "[network][squad][assets]")
{
    ...
    REQUIRE(Poseidon::BuildNetworkSquadPictureDownloadUrl(
                RString("https://example.com/squads/raw/squad.xml"),
                RString("synthetic_grid.paa")) ==
            RString("https://example.com/squads/raw/"
                    "synthetic_grid.paa"));
    REQUIRE(Poseidon::BuildNetworkSquadPictureDownloadUrl(
