...
TEST_CASE("MP join - 3.02 is rejected before player admission side effects", "[version][mpjoin][network]")
{
    ...
    REQUIRE(botId == -1);
    // Without a queued CreatePlayerInfo, NetworkServer::OnPlayerCreate is never called,
    // so no player record or per-player dynamic message formats can be registered.
}
