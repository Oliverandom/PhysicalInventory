...
#include <Poseidon/Graphics/Textures/TextureBank.hpp>       // GlobLoadTexture (fuel-station icon)
...
#include <Poseidon/World/Entities/Infantry/SoldierOld.hpp>   // Man, ManPos, GetActUpDegree
#include <Poseidon/World/Entities/Weapons/ItemMass.hpp>      // TotalCarried (encumbrance bar)
#include <Poseidon/IO/ParamFileExt.hpp>                      // GetPictureName (fuel-station icon)
...
RString UIAction::GetDisplayName(AIUnit* unit) const
{
    ...
    switch (type)
    {
        ...
        case ATHeal:
            ...
            else if (param == kMedkitHealParam)
            {
                // First-aid-kit heal of another person: label by their side.
                const char* who = "soldier";
                switch (target->GetTargetSide())
                {
                    case TCivilian: who = "civilian"; break;
                    case TGuerrila: who = "guerilla"; break;
                    default:        who = "soldier";  break;   // West / East
                }
                snprintf(buffer, sizeof(buffer), "Heal %s", who);
            }
            else
            {
        ...
        case ATRefuel:
            ...
            else if (param == kJerryRefuelParam)
            {
                snprintf(buffer, sizeof(buffer), "Refuel %s", (const char*)target->GetDisplayName());
            }
            else if (param == kJerrySyphonParam)
            {
                snprintf(buffer, sizeof(buffer), "Syphon Fuel from %s",
                         (const char*)target->GetDisplayName());
            }
            else
            {
...
// --- Player stamina + health bars (bottom-right) ---------------------------
// Stamina here is just a READ-OUT of the soldier's existing fatigue value (Man::
// _tired): stamina = 1 - tired. The drain/recharge schedule and its effects (sprint
// falloff, panting) live in the soldier sim (SoldierOldSim.cpp), so there is a single
// source of truth and no parallel system.
// Edit-menu settings (defined in InventoryModel.cpp) controlling the HUD bars.
namespace Poseidon { bool InventoryShowBars(); float InventoryBarOpacity();
                     bool InventoryShowFuelHud(); float InventoryFuelHudOpacity(); }

// Vehicle fuel bar: the white fuel-station map icon, a colour-banded bar (green >= 50%,
// amber >= 10%, red < 10%) and "cur/max L" text. `ax,ay` is the abs top-left corner.
static void DrawVehicleFuelBar(EntityAI* veh, float ax, float ay, float alpha, Ref<Font> font,
                               bool showSpeed)
{
    if (!veh || !veh->GetType() || veh->GetType()->GetFuelCapacity() <= 0.0f)
        return;
    // The in-vehicle fuel+speed HUD (showSpeed) obeys the debug-menu show toggle and
    // opacity slider. The jerry-can refuel-target mini-bar (showSpeed == false) is a
    // separate interaction readout and always shows.
    if (showSpeed)
    {
        if (!InventoryShowFuelHud())
            return;
        alpha *= InventoryFuelHudOpacity();
    }
    const float cap = veh->GetType()->GetFuelCapacity();
    float liters = veh->GetFuel();
    saturate(liters, 0.0f, cap);
    const float frac = (cap > 0.0f) ? liters / cap : 0.0f;

    const int W = GLOB_ENGINE->Width2D();
    const int H = GLOB_ENGINE->Height2D();
    Texture* white = GLOB_SCENE->Preloaded(TextureWhite);
    MipInfo wmip = GLOB_ENGINE->TextBank()->UseMipmap(white, 0, 0);

    const float cy    = ay + 0.008f;                 // vertical centre of the readout (abs)
    const float icoPx = 0.026f * H;                  // square fuel-station icon
    const float barCY = cy * H;                      // bar centre (px)
    const float x0    = ax * W;
    const float barX  = x0 + icoPx + 0.006f * W;
    const float barH  = 0.006f * H;                  // thin bar

    char litersTxt[32];
    snprintf(litersTxt, sizeof(litersTxt), "%.0f/%.0f L", liters, cap);
    const float litSize = 0.024f;   // litres readout font (between the old 0.022 and 0.026)
    const float litWfrac = GLOB_ENGINE->GetTextWidth(litSize, font.GetRef(), litersTxt);

    // In-vehicle HUD (showSpeed): a long bar running to the right edge of the screen,
    // with km/h above its LEFT end and the litres above its RIGHT end. The jerry-can
    // refuel-target mini-bar keeps a short fixed bar with the litres to its right.
    const float rightEdge = 0.985f;                  // near the screen's right edge
    float barW, litLeftFrac;
    if (showSpeed)
    {
        barW = rightEdge * W - barX;                 // stretch to the right edge
        litLeftFrac = rightEdge - litWfrac;          // litres right-aligned to the edge
    }
    else
    {
        barW = 0.130f * W;                           // compact bar (a bit longer)
        litLeftFrac = (barX + barW) / W + 0.008f;    // litres just to its right
    }
    if (barW < 0.05f * W) barW = 0.05f * W;

    // Pre-whitened fuel-station map icon (recoloured copy of data\fuelstation.paa).
    static Ref<Texture> fuelIcon = GlobLoadTexture("invicons\\fuelstation_white.paa");
    if (fuelIcon)
    {
        MipInfo mip = GLOB_ENGINE->TextBank()->UseMipmap(fuelIcon.GetRef(), 0, 0);
        GLOB_ENGINE->Draw2D(mip, PackedColor(Color(1.0f, 1.0f, 1.0f, alpha)),
                            Rect2DPixel(x0, barCY - icoPx * 0.5f, icoPx, icoPx));
    }

    const PackedColor bg(Color(0.0f, 0.0f, 0.0f, 0.55f * alpha));
    const PackedColor fill = (frac < 0.10f) ? PackedColor(Color(0.85f, 0.20f, 0.20f, alpha))  // red
                           : (frac < 0.50f) ? PackedColor(Color(0.92f, 0.66f, 0.15f, alpha))  // amber
                                            : PackedColor(Color(0.20f, 0.78f, 0.28f, alpha));  // green
    GLOB_ENGINE->Draw2D(wmip, bg,   Rect2DPixel(barX, barCY - barH * 0.5f, barW, barH));
    GLOB_ENGINE->Draw2D(wmip, fill, Rect2DPixel(barX, barCY - barH * 0.5f, barW * frac, barH));

    const PackedColor wc(Color(1.0f, 1.0f, 1.0f, alpha));
    if (showSpeed)
    {
        // Horizontal ground speed (ignore vertical bounce). NOTE: this is the vehicle's
        // TRUE velocity - OFP's own dashboard gauges are calibrated low, so this can read
        // higher than the in-cockpit needle.
        Vector3Val sp = veh->ObjectSpeed();
        const int kmh = (int)(sqrtf(sp.X() * sp.X() + sp.Z() * sp.Z()) * 3.6f + 0.5f);
        char spTxt[24];
        snprintf(spTxt, sizeof(spTxt), "%d km/h", kmh);
        // Anchor the km/h readout near the LEFT end of the bar, right-aligned to a fixed
        // point that leaves exactly enough room for a 3-digit speed. Right-aligning keeps
        // the "km/h" label (and the number's right edge) perfectly still as the speed
        // changes - the digits just grow leftward - instead of the text drifting. The
        // proportional font made a padded "%3d" still jitter, so we measure and place it.
        const float kmhSize   = 0.032f;   // speed readout font (slightly bigger)
        const float threeW    = GLOB_ENGINE->GetTextWidth(kmhSize, font.GetRef(), "999 km/h");
        const float kmhRightX = barX / W - 0.006f + threeW;   // aligned near the fuel icon
        const float kmhW      = GLOB_ENGINE->GetTextWidth(kmhSize, font.GetRef(), spTxt);
        GLOB_ENGINE->DrawText(Point2DFloat(kmhRightX - kmhW, cy - 0.040f), kmhSize, font, wc, spTxt);
        // Litres above the RIGHT end of the bar (nudged slightly down).
        GLOB_ENGINE->DrawText(Point2DFloat(litLeftFrac, cy - 0.035f), litSize, font, wc, litersTxt);
    }
    else
    {
        // Litres only, vertically centred on the compact bar.
        GLOB_ENGINE->DrawText(Point2DFloat(litLeftFrac, cy - 0.011f), 0.022f, font, wc, litersTxt);
    }
}

// Total vertical extent of the two-bar block (health row + gap + stamina row), used
// by the caller to lift the block above the command bar when it is showing.
static const float kHSBarBlockH = 0.0060f + 0.004f + 0.0060f;

static void DrawHealthStaminaBars(Vehicle* person, float healthRowY)
{
    if (!person || !InventoryShowBars())
        return;

    Man* man = dyn_cast<Man>(person);

    // Health = how close to death. GetTotalDammage() is an average over ALL hit points
    // (legs/arms/hands/head/body), so a lethal torso/head shot barely moves it. A
    // soldier actually dies when the BODY or HEAD hit point maxes out, so drive the bar
    // off the worst of those (with total as a floor for general wear).
    float damage = person->GetTotalDammage();
    if (man)
    {
        const float body = man->GetBodyHit();
        const float head = man->GetHeadHit();
        if (body > damage) damage = body;
        if (head > damage) damage = head;
    }
    float health = 1.0f - damage;
    if (health < 0.0f) health = 0.0f;
    if (health > 1.0f) health = 1.0f;

    float stamina = man ? (1.0f - man->GetTired()) : 1.0f;
    if (stamina < 0.0f) stamina = 0.0f;
    if (stamina > 1.0f) stamina = 1.0f;

    // --- draw the two thin bars, bottom-right (no text) -------------------
    Texture* whiteTex = GLOB_SCENE->Preloaded(TextureWhite);
    MipInfo mip = GLOB_ENGINE->TextBank()->UseMipmap(whiteTex, 0, 0);
    const int w = GLOB_ENGINE->Width2D();
    const int h = GLOB_ENGINE->Height2D();

    const float bx = 0.820f;   // left edge of the bars
    const float bw = 0.150f;   // full width
    const float bh = 0.0060f;  // thin
    const float hy = healthRowY;         // health row (positioned by the caller)
    const float sy = hy + bh + 0.004f;   // stamina row just below

    const float op = InventoryBarOpacity();   // edit-menu bar opacity (default 0.90)
    const PackedColor bg(Color(0.0f, 0.0f, 0.0f, 0.55f * op));
    const PackedColor green(Color(0.20f, 0.78f, 0.28f, 0.95f * op));
    const PackedColor grey(Color(0.62f, 0.64f, 0.66f, 0.95f * op));

    GLOB_ENGINE->Draw2D(mip, bg,    Rect2DPixel(bx * w, hy * h, bw * w, bh * h));
    GLOB_ENGINE->Draw2D(mip, green, Rect2DPixel(bx * w, hy * h, bw * health * w, bh * h));
    GLOB_ENGINE->Draw2D(mip, bg,    Rect2DPixel(bx * w, sy * h, bw * w, bh * h));
    GLOB_ENGINE->Draw2D(mip, grey,  Rect2DPixel(bx * w, sy * h, bw * stamina * w, bh * h));
}
...
void InGameUI::DrawUnitInfo(EntityAI* vehicle)
{
    ...
    // Bottom-right readout: fuel bar + speed (km/h) while in ANY vehicle - drawn HERE,
    // before the "car has no info" early-out below, so cars/trucks show it too. On foot
    // it's the health + stamina bars instead. Lifts above the command bar when it's up.
    {
        float barY = 0.930f;
        const bool cmdBarUp = _showGroupInfo && unit->IsGroupLeader() &&
                              group && group->NUnits() > 1;
        if (cmdBarUp)
            barY = uiY - 0.014f - kHSBarBlockH;

        Transport* inVeh = unit->GetVehicleIn();   // vehicle you occupy (crew or cargo)
        if (inVeh)
        {
            // In a ground vehicle: fuel bar + speed (bottom-right). Aircraft are skipped -
            // they have their own cockpit instruments.
            const char* sim = inVeh->GetType() ? (const char*)inVeh->GetType()->_simName : nullptr;
            const bool aircraft = sim && (strcmp(sim, "airplane") == 0 ||
                                          strcmp(sim, "helicopter") == 0);
            if (!aircraft && inVeh->GetType() && inVeh->GetType()->GetFuelCapacity() > 0.0f)
                DrawVehicleFuelBar(inVeh, 0.775f, barY, 1.0f, _font24, /*showSpeed*/ true);
        }
        else
        {
            DrawHealthStaminaBars(unit->GetPerson(), barY);
            if (Man* pl = dyn_cast<Man>(vehicle))
                if (pl->FuelDir() != 0)
                    DrawVehicleFuelBar(dyn_cast<Transport>(pl->FuelTarget()), 0.020f, 0.034f,
                                       1.0f, _font24, /*showSpeed*/ false);
        }
    }
    ...
    if (weapon >= 0 && weapon < vehicle->NMagazineSlots())
    {
        ...
        // Differentiate the Put items in the weapon indicator: the AP mine (MineE) and
        // AT mine (Mine) both carry the config display name "Mine", so they'd otherwise
        // read identically when selected.
        if (magazine && magazine->_type)
        {
            const char* cn = (const char*)magazine->_type->GetName();
            if (strcmp(cn, "MineE") == 0)         displayName = RString("AP Mine");
            else if (strcmp(cn, "Mine") == 0)     displayName = RString("AT Mine");
            else if (strcmp(cn, "PipeBomb") == 0) displayName = RString("Satchel Charge");
            else if (strcmp(cn, "TimeBomb") == 0) displayName = RString("Timed Satchel");
        }
