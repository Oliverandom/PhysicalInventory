...
#include <cstring>   // strcmp (vehicle _simName check)
...
void InGameUI::DrawHUD(const Camera& camera, EntityAI* vehicle, CameraType cam)
{
    ...
    {
        // Stack the three bottom HUD "tables" with even gaps when the command bar is
        // up: command bar (top edge uiY) at the bottom, the health/stamina bars just
        // above it, and the action menu above those. Keep the constants in sync with
        // DrawHealthStaminaBars / DrawUnitInfo (gap 0.014, bar block 0.016).
        const bool cmdBarUp = _showGroupInfo && unit->IsGroupLeader() &&
                              group && group->NUnits() > 1;
        // In a ground vehicle the bottom readout is the fuel bar, which draws its km/h +
        // litres text ABOVE the bar (unlike the on-foot health/stamina bars). Detect that
        // so the action menu can be lifted extra to clear the text. Aircraft draw no such
        // readout, so they need no extra clearance.
        bool groundVehReadout = false;
        if (Transport* inVeh = unit->GetVehicleIn())
        {
            const char* sim = inVeh->GetType() ? (const char*)inVeh->GetType()->_simName : nullptr;
            const bool aircraft = sim && (strcmp(sim, "airplane") == 0 || strcmp(sim, "helicopter") == 0);
            groundVehReadout = !aircraft;
        }
        if (cmdBarUp)
        {
            const float gap = 0.014f;       // even spacing between the tables
            const float barBlockH = 0.016f; // health + gap + stamina row block
            const float barTop = uiY - gap - barBlockH;
            float anchor = barTop - gap;
            if (groundVehReadout)
                anchor -= 0.040f;   // clear the km/h + litres text sitting above the bar
            _actions.SetBottomAnchor(anchor);
        }
        else
        {
            // Default anchor, raised so it clears the fuel/speed HUD (extra when in a
            // ground vehicle, whose readout text sits higher above the bar).
            _actions.SetBottomAnchor(groundVehReadout ? 0.79f : 0.82f);
        }
        _actions.OnDraw();
    }
