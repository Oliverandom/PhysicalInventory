...
const char* KbmPage::ActionLabelOverride(UserAction action, ControlsCategory category) const
{
    if (action == UAInventory)
        return "Inventory";
    if (action == UAInventoryEditToggle)
        return "Inventory Edit Menu";
    return BindingsPage::ActionLabelOverride(action, category);
}
