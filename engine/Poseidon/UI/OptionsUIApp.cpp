...
DisplayMain::DisplayMain(ControlsContainer* parent) : Display(parent)
{
    ...
    if (GetCtrl(IDC_MAIN_MODS) == nullptr && GameModuleRegistry::FindByIDC(IDC_MAIN_MODS) != nullptr)
    {
        ...
        if (quit != nullptr && quitCls != nullptr && quitCls->IsClass())
        {
            ...
            {
                // Preferred (retail) layout: stack MODS directly beneath OPTIONS, one row
                // down, reusing the MISSION EDITOR -> OPTIONS spacing so it lines up with
                // that column instead of sitting beside QUIT.
                Control* opt = dynamic_cast<Control*>(GetCtrl(IDC_MAIN_OPTIONS));
                Control* edt = dynamic_cast<Control*>(GetCtrl(IDC_MAIN_EDITOR));
                bool stacked = false;
                if (opt != nullptr && opt->IsVisible())
                {
                    const float dy = (edt != nullptr) ? (opt->Y() - edt->Y()) : 0.0f;
                    const float step = (dy > 0.0001f) ? dy : opt->H() * 1.5f;
                    mods->SetPos(opt->X(), opt->Y() + step, opt->W(), opt->H());
                    stacked = true;
                }

                if (!stacked)
                {
                    // Fallback: the geometric helper (beside Quit) when OPTIONS isn't present.
