...
#include <Poseidon/UI/Inventory/DisplayInventory.hpp>
...
void DisplayMission::OnSimulate(EntityAI* vehicle)
{
    ...
    */
    ...
    // Trickle inventory-icon preloading during normal gameplay frames so they are
    // resident before the inventory is opened (no open/page-flip hitch). Self-guards
    // once complete; a few per frame keeps it invisible.
    PreloadInventoryIconsStep(6);

    // Keep the laser designator's battery topped up so it never runs out - it just
    // emits a laser. Single-player only to avoid touching MP ammo synchronisation.
    if (GWorld->GetMode() != GModeNetware)
    {
        Person *desig = GWorld->PlayerOn();
        if (!desig)
            desig = GWorld->GetRealPlayer();
        if (desig)
            KeepLaserDesignatorLoaded(desig);
    }

    // Inventory (bound via Options > Keyboard & Mouse). Single-player / editor only
    // for now - the transfer path still needs network routing before it is MP-safe.
    if (GWorld->GetMode() != GModeNetware && GWorld->GetEndMode() == EMContinue && !Child())
    {
        auto &in = InputSubsystem::Instance();
        // Toggle the inventory edit menu (debug sliders) via its bound key.
        if (in.GetActionToDo(UAInventoryEditToggle))
            SetInventoryEditMenuVisible(!InventoryEditMenuVisible());

        if (in.GetActionToDo(UAInventory))
        {
            OpenInventory();
        }
        else
        {
            // Weapon hotkeys 1-9,0: switch to the bound weapon if carried. Only
            // consumed for slots that actually have a binding, so unbound number
            // keys still reach the command menu.
            static const int kNumKeys[10] = {
                SDL_SCANCODE_1, SDL_SCANCODE_2, SDL_SCANCODE_3, SDL_SCANCODE_4, SDL_SCANCODE_5,
                SDL_SCANCODE_6, SDL_SCANCODE_7, SDL_SCANCODE_8, SDL_SCANCODE_9, SDL_SCANCODE_0 };
            for (int s = 0; s < 10; s++)
            {
                if (GetWeaponHotkey(s).GetLength() == 0)
                    continue;
                if (in.GetKeyToDo(kNumKeys[s]))
                {
                    Person *p = GWorld->PlayerOn();
                    if (!p)
                        p = GWorld->GetRealPlayer();
                    ApplyWeaponHotkey(p, s);
                    break;
                }
            }
        }
...
void DisplayMission::OpenInventory()
{
    if (Child() || GetMsgBox())
        return;

    // The unit the player is currently controlling; fall back to their own
    // soldier. Null in a pure camera/cutscene state - just don't open then.
    Person *player = GWorld->PlayerOn();
    if (!player)
        player = GWorld->GetRealPlayer();
    if (player)
        CreateChild(new DisplayInventory(this, player));
}
