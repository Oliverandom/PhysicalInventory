    ...
    if (!knownObject)
    {
        ...
        for (int i = 0; i < _pendingChangeOwners.Size(); i++)
        {
            if (_pendingChangeOwners[i].object == co.object)
            {
                ...
                LOG_INFO(Network, "ChangeOwner: updated pending object {}:{} owner={}", co.object.creator,
                         co.object.id, co.owner);
                return false;
            }
        }
            ...
            switch (gs.gameState)
            {
                ...
                case NGSPlay:
                    ...
                    if (_state == NGSBriefing ||
                        (_jip && _missionHeader.joinInProgress && _state >= NGSLoadIsland))
                    {
                        if (_state != NGSBriefing)
                        {
