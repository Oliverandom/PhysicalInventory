...
void AppConfig::ParseCommandLine(int argc, char** argv)
{
    ...
    try
    {
        ...
        const std::string versionForVersionFlag =
            (const char*)Poseidon::GetVersionStringForState(ContainsCliArg(normalizedArgs, "--dev"),
                                                            GApp != nullptr && GApp->IsDemo());
        ...
        showOption(multiplayerGroup->add_flag("--force-jip", _forceJIP,
                   ...
                   CliHelpVisibility::Dev);
        multiplayerGroup->add_flag("--mp-version",
                                   _printMPVersion,
