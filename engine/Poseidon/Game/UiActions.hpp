...
public:
	...
	//! Bottom edge (0..1) the action menu grows upward from. Adjusted per-frame so the
	//! menu clears the health/stamina bars + command bar when they are on screen.
	void  SetBottomAnchor(float b) { _bottom = b; }
	float GetBottomAnchor() const  { return _bottom; }
