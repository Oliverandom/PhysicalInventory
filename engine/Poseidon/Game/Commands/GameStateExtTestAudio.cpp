...
GameValue TriSetPlayerFaceView(const GameState* /*state*/, GameValuePar arg)
{
    ...
    const float distance =
        a.Size() >= 1 ? static_cast<float>(static_cast<GameScalarType>(a[0])) : 1.2f;
    const float height =
        a.Size() >= 2 ? static_cast<float>(static_cast<GameScalarType>(a[1])) : 1.55f;
...
{
    return GameValue(static_cast<GameScalarType>(
        InputSubsystem::Instance().GetAction(UAVoiceOverNetPushToTalk, false)));
}
