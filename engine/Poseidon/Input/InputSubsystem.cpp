...
// While the inventory UI is open, the RIGHT mouse button (index 1) is suppressed
// from driving any keybound UserAction, so right-clicking in that UI can't also
// fire a gameplay bind. Set each frame the inventory is open; cleared on close.
static bool g_suppressRmbActions = false;
static inline bool RmbActionSuppressed(int index) { return g_suppressRmbActions && index == 1; }
...
static bool ModifierHeld(const Input& in, UserAction action, int slot)
{
    ...
    switch (InputBindingDevice(mod))
    {
        ...
        case INPUT_DEVICE_MOUSE:
            return value >= 0 && value < N_MOUSE_BUTTONS && !RmbActionSuppressed(value) &&
                   in.mouse.buttons[value] > 0.0f;
...
void InputSubsystem::SetSuppressRightButtonActions(bool suppress)
{
    g_suppressRmbActions = suppress;
}
...
// Encumbrance look-scale: the soldier sim sets this each frame (1.0 normally, lower
// under a heavy pack). It's applied to the AIM delta inside MouseState::Update (which
// is what actually turns the view), NOT here - the classic aim path never reads
// GetMouseDeltaX. The UI cursor is left untouched so menus stay responsive.
static float g_playerLookScale = 1.0f;
void  InputSubsystem::SetPlayerLookScale(float s)
{
    g_playerLookScale = s;
    GInput.mouse.aimLookScale = s;   // consumed by MouseState::Update
}
float InputSubsystem::GetPlayerLookScale() const  { return g_playerLookScale; }
...
#endif
        // Inventory system (labels overridden in KbmPage; desc reuses an existing
        // string id only so LocalizeString has a valid handle). Defaults chosen to
        // avoid clashing with any stock binding (I is taken by NetworkStats, Tab by
        // LockTargets): O opens the inventory, K toggles the edit/tuning menu. Both are
        // rebindable in Options -> Controls.
        UserActionDesc("Inventory", IDS_USRACT_MAP, SDL_SCANCODE_O, -1),
        UserActionDesc("InventoryEditMenu", IDS_USRACT_HELP, SDL_SCANCODE_K, -1),
    };
