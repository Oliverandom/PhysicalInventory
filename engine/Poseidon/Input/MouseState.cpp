                        ...
                        const CursorClamp* clamp, float cursorAspectRatio, float aimAspectRatio)
{
    ...
    if (gameFocusLost <= 0)
    {
        // Encumbrance scales the AIM (look) delta only - the UI cursor below is left
        // full-speed so menus stay responsive under a heavy pack.
        cursor.aimDeltaX += aimMoveX * aimLookScale;
        if (reverseY)
            cursor.aimDeltaY -= moveY * aimLookScale;
        else
            cursor.aimDeltaY += moveY * aimLookScale;
    }
