...
struct MouseState
{
    ...
    // Encumbrance AIM scale (1.0 = normal). Set from the soldier sim via
    // InputSubsystem::SetPlayerLookScale; scales the look/aim delta only (not the UI
    // cursor), so a heavy pack makes aiming sluggish.
    float aimLookScale = 1.0f;
