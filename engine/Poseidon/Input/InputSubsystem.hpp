...
class InputSubsystem
{
  public:
    ...
    // While true, the right mouse button drives no keybound UserAction (the
    // inventory UI sets this so its own right-click can't also fire a gameplay
    // bind). Does not affect IsMouseRightDown(), which the UI reads directly.
    void SetSuppressRightButtonActions(bool suppress);

    // Encumbrance look-scale (1.0 = normal). Set by the soldier sim from carried
    // weight so mouse-look aiming slows under a heavy pack; scales the look deltas only.
    void  SetPlayerLookScale(float s);
    float GetPlayerLookScale() const;
