...
// True for the runtime-injected first-aid-kit magazines (RegisterMedkits,
// GameState.cpp). Using one is a full self-heal that consumes one of the kit's
// three uses, rather than the default reload-and-fire magazine behaviour.
bool IsFirstAidKitMagazine(const char* name);

// Sentinel stored in UIAction::param to tag a first-aid-kit "heal other" ATHeal
// action (a soldier heals a wounded person with their own medkit), so it is handled
// separately from the vanilla attendant-vehicle heal (which uses param 0).
constexpr int kMedkitHealParam = 0x4D4B4854; // 'MKHT'

// True for the runtime-injected jerry-can fuel magazine (RegisterMedkits, GameState).
bool IsJerryCanMagazine(const char* name);
// True for a gas-station object class (map fuel stations), an infinite fuel source.
bool IsFuelStationClass(const char* name);

// Sentinels in UIAction::param tagging the jerry-can fuel actions on a vehicle/station:
// pour the can into the vehicle, or syphon the vehicle/station into the can. Handled in
// Man::StartActionProcessing (starts a timed transfer run in the soldier sim).
constexpr int kJerryRefuelParam = 0x4A524546; // 'JREF'
constexpr int kJerrySyphonParam = 0x4A535950; // 'JSYP'
...
class EntityAI: public Entity
{
	...
	int _lastRealWeapon = -1; //!< last selected rifle/launcher/pistol slot, so a depleted
	                          //!< throwable can switch back to it instead of another grenade
