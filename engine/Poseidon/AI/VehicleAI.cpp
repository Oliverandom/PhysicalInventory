...
{
    return name && (stricmp(name, "US_FirstAidKit") == 0 ||
                    stricmp(name, "AI2_FirstAidKit") == 0);
}

bool IsJerryCanMagazine(const char* name)
{
    return name && stricmp(name, "JerryCan") == 0;
}
...
bool IsFuelStationClass(const char* name)
{
    if (!name)
        return false;
    // Match the several map gas-station classes (FuelStation, FuelStationNew,
    // fuelstation_army, ...): any class whose name contains "fuelstation".
    char low[64];
    int i = 0;
    for (; name[i] && i < 63; i++)
    {
    ...
    low[i] = 0;
    return strstr(low, "fuelstation") != nullptr;
}
...
void EntityAI::SelectWeapon(int weapon, bool changed)
{
    ...
    if (_currentWeapon >= 0 && _currentWeapon < NMagazineSlots())
    {
        ...
        if (weaponType)
        {
            ...
            // Remember the last real weapon (rifle/launcher/pistol) so that when a
            // throwable runs out we can switch back to it rather than cross-loading
            // another grenade/mine into the shared Throw/Put muzzle.
            if (mask & (MaskSlotPrimary | MaskSlotSecondary | MaskSlotHandGun))
            {
                _lastRealWeapon = _currentWeapon;
            }
        }
    }
}
