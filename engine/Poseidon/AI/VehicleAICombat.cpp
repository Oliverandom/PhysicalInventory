...
bool EntityAI::FireWeapon(int weapon, TargetType* target)
{
    ...
    if (magazine->_ammo <= 0)
    {
        // Throw/Put pseudo-weapons (grenades/mines/satchels) share ONE muzzle across
        // several explosive types, so vanilla AutoReload cross-loads the next type -
        // using your last AP mine silently arms a satchel. For the human player, when a
        // throwable runs out and none of the SAME type remains, switch back to the last
        // real weapon (rifle/launcher/pistol) instead of cross-loading a different one.
        const MagazineSlot &spentSlot = GetMagazineSlot(weapon);
        const bool throwMuzzle = spentSlot._weapon &&
            (spentSlot._weapon->_weaponType &
                (MaskSlotPrimary | MaskSlotSecondary | MaskSlotHandGun)) == 0;
        bool sameTypeLeft = false;
        if (throwMuzzle && magazine->_type)
        {
            for (int j = 0; j < NMagazines(); j++)
            {
                Magazine *mg = GetMagazine(j);
                if (mg && mg != magazine && mg->_type == magazine->_type && mg->_ammo > 0)
                {
                    sameTypeLeft = true;
                    break;
                }
        ...
        const bool isPlayer = IsLocal() && GWorld && GWorld->GetRealPlayer() == this;

        if (throwMuzzle && !sameTypeLeft && isPlayer)
        {
            RemoveMagazine(magazine);   // drop the spent throwable (don't cross-load)
            int back = (_lastRealWeapon >= 0 && _lastRealWeapon < NMagazineSlots())
                           ? _lastRealWeapon
                           : FirstWeapon();
            SelectWeapon(back, true);
        }
        else
        {
