...
#include <Poseidon/World/Entities/Infantry/SoldierOld.hpp>   // Man, ConsumeOneMedkitUse (medkit heal-other)
...
void EntityAI::PerformAction(const UIAction& action, AIUnit* unit)
{
    switch (action.type)
    {
        ...
        case ATUseMagazine:
        {
            ...
            // First-aid kit: full self-heal, consume one of the three uses, and
            // remove the kit from the inventory once it is spent. No muzzle can
            // load it, so it never takes the reload-and-fire path below.
            if (IsFirstAidKitMagazine(magazine->_type->GetName()))
            {
                Repair(1.0f);
                magazine->_ammo -= 1;
                if (magazine->_ammo <= 0)
                {
                    Ref<Magazine> dead = magazine;
        ...
        case ATUserType:
            ...
            return;
        case ATHeal:
            // The first-aid-kit heal of another person is applied directly in
            // Man::StartActionProcessing (heal + kit consumption), and the vanilla
            // attendant-vehicle heal is handled by the supply simulation. Nothing to do
            // here; just swallow ATHeal so it doesn't log as an unknown action.
            return;
    }
...
void EntityAI::GetActions(UIActions& actions, AIUnit* unit, bool now)
{
    ...
    // Jerry-can fuelling: when the looking player carries a jerry can and THIS object is
    // a fuel-holding vehicle or a gas station, offer "Refuel" (pour the can in) and/or
    // "Syphon Fuel from" (draw fuel out into the can). Both start a timed transfer in
    // Man::StartActionProcessing (see the ATRefuel + kJerry* handling).
    if (unit->GetVehicle() != this && unit->GetPerson())
    {
        Man* looker = dyn_cast<Man>(unit->GetPerson());
        const EntityAIType* et = GetType();
        const bool station = et && IsFuelStationClass(et->GetName());
        const float cap = et ? et->GetFuelCapacity() : 0.0f;
        const float reach = GetRadius() + 4.0f;   // must be right at the vehicle/station
        const bool inRange = looker &&
            looker->Position().DistanceXZ2(Position()) <= reach * reach;
        if (inRange && (cap > 0.0f || station))
        {
            if (!station && looker->FindJerryCan(false) && GetFuel() < cap - 0.001f)
                actions.Add(ATRefuel, this, 0.48, kJerryRefuelParam, true, true);
            if (looker->FindJerryCan(true) && (station || GetFuel() > 0.001f))
                actions.Add(ATRefuel, this, 0.47, kJerrySyphonParam, true, true);
        }
    }
    ...
    if (unit->GetVehicle() == this && CommanderUnit() == unit && !unit->IsInCargo())
    {
        ...
        if (EnableWeaponManipulation())
        {
            ...
            for (int i = 0; i < _magazines.Size(); i++)
            {
                ...
                {
                    // First-aid kit: only offer "Use first aid kit" when actually hurt.
                    // Uses a small tolerance rather than == 0 so trivial residual damage
                    // (a bump, a scrape) doesn't keep the action up when you read as full
                    // health; a real wound is well above this.
                    if (IsFirstAidKitMagazine(magazine->_type->GetName()) &&
                        GetTotalDammage() < 0.02f)
                    {
                        continue;
                    }
