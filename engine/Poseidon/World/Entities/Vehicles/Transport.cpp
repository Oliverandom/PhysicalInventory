...
static bool IsSupply(EntityAI* vehicle)
{
    ...
    // The field hospital (MASH tent) / hospital building have no cargo/attendant
    // config, so they would never build a ResourceSupply. Treat them as supply
    // containers so they can hold (and display) the first-aid kits we stock in.
    {
        const char* nm = (const char*)type->GetName();
        if (strcmpi(nm, "MASH") == 0 || strcmpi(nm, "hospital") == 0)
        {
            return true;
        }
    }
    // Military cars/trucks and tanks carry jerry cans, so expose them as supply
    // containers (the cans are stocked in the ResourceSupply constructor).
    {
        const char* sim = (const char*)type->_simName;
        if (sim && (strcmpi(sim, "car") == 0 || strcmpi(sim, "tank") == 0) &&
            type->GetTypicalSide() != TCivilian)
        {
            return true;
        }
    }
...
ResourceSupply::ResourceSupply(EntityAI* vehicle) : _parent(vehicle)
{
    ...
    // The M113 ambulance, BMP ambulance and field hospital come stocked with 12
    // first-aid kits (US kits for the M113 and the hospital, AI-2 for the BMP).
    // Forced into cargo (deleteWhenFull=true) so they fit even though these medic
    // vehicles have no configured magazine-cargo capacity, and show as lootable
    // items in the vehicle's inventory.
    {
        RString cls = type->GetName();
        const char* kit = nullptr;
        if (strcmpi((const char*)cls, "M113Ambul") == 0)     kit = "US_FirstAidKit";
        else if (strcmpi((const char*)cls, "BMPAmbul") == 0) kit = "AI2_FirstAidKit";
        else if (strcmpi((const char*)cls, "MASH") == 0)     kit = "US_FirstAidKit";
        else if (strcmpi((const char*)cls, "hospital") == 0) kit = "US_FirstAidKit";
        if (kit)
        {
            Ref<MagazineType> mt = MagazineTypes.New(kit);
            if (mt)
                AddMagazineCargo(mt, 12, true);
        }
    }

    // Military cars/trucks carry one full jerry can; tanks carry two.
    {
        const char* sim = (const char*)type->_simName;
        int cans = 0;
        if (sim && type->GetTypicalSide() != TCivilian)
        {
            if (strcmpi(sim, "car") == 0)       cans = 1;
            else if (strcmpi(sim, "tank") == 0) cans = 2;
        }
        if (cans > 0)
        {
            Ref<MagazineType> jc = MagazineTypes.New("JerryCan");
            if (jc)
                AddMagazineCargo(jc, cans, true);
        }
    }
    _action = ATNone;
    _actionParam = 0;
