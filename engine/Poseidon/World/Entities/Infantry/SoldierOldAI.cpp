...
bool Soldier::FireWeapon(int weapon, TargetType* target)
{
    ...
    switch (mode->_ammo->_simulation)
    {
        ...
        case AmmoShotMine:
        {
            ...
            if (fired)
            {
                // Play the vanilla put-down gesture when a mine/satchel is deployed by
                // firing. The action-menu Put path (ATFirePutDown) animates, but this
                // direct-fire case never did - so deploying via a hotkey/fire looked
                // static. Same ManActPutDown the action menu uses.
                PlayAction(ManActPutDown);
                if (mode->_ammo->_simulation == AmmoShotPipeBomb)
                {
