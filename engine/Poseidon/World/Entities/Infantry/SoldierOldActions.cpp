...
#include <Poseidon/Game/TitEffects.hpp>   // CreateTitleEffect / TitPlain (encumbered popup)
#include <stdio.h>
#include <cmath>
...
void Soldier::KeyboardPilot(float deltaT, SimulationImportance prec)
{
    ...
    // First-aid-kit self-heal is cancellable: if the medic animation is queued
    // and the player presses a movement key, drop the queued gesture so we fall
    // straight through into normal walking THIS frame - and don't spend a kit.
    // Done before AdvanceExternalQueue so the medic move isn't re-latched below.
    if (_medkitHealPending)
    {
        const bool moving = fabs(input.GetMoveForward()) + fabs(input.GetMoveBack()) +
                            fabs(input.GetMoveLeft()) + fabs(input.GetMoveRight()) +
                            fabs(input.GetMoveFastForward()) + fabs(input.GetMoveSlowForward()) > 0.01f;
        if (moving)
        {
            _medkitHealPending = false;                       // cancelled - no kit used
            _forceMove = MotionPathItem((MoveId)MoveIdNone);
            _externalQueue.Clear();                           // drop queued ManActMedic/ManActUp
    ...
    // While a blocking UI (the inventory screen) is open we still let the player
    // walk around with the movement keys, but suppress everything else - no
    // aiming/ADS, firing, weapon changes, reloading, binoculars or stance
    // changes - since the mouse belongs to the inventory panes. The weapon/aim
    // path lives outside KeyboardPilot and stays gated, so it remains disabled;
    // here we run only the movement selection, then return.
    if (!GWorld->IsUIEnabled())
    {
        // Fold the A/D body turn (computed above) into _turnWanted smoothly.
        {
        ...
        const float fbWanted = ((input.GetMoveFastForward() > 0) * 2.0f + (input.GetMoveForward() > 0) * 1.0f +
                                (input.GetMoveSlowForward() > 0) * 0.5f + (input.GetMoveBack() > 0) * -1.0f);
        const float rlWanted = input.GetAction(UATurnRight) - input.GetAction(UATurnLeft);
        const bool turboUI = input.GetAction(UATurbo) > 0.5f;

        bool forceStandUI = false;
        const float slopeUI = LandSlope(forceStandUI);
        ActionMap* mapUI = Type()->GetActionMap(_primaryMove.id);

        bool wantSlowUI = false;
        if (GWorld->FocusOn() == unit)
            wantSlowUI = _walkToggle || GWorld->GetCameraTypeWanted() == CamGunner;

        int fastFlag = turboUI ? 1 : 0;
        ...
        {
            if (!_canMoveFast || !CanSprint(slopeUI))
                fastFlag = 0;
            fastFlag += 1;
            if ((wantSlowUI && fastFlag == 1) || !CanRun(slopeUI))
                fastFlag = 0;
        }
        if ((_sprintExhausted || _tooHeavyToRun) && !IsDown())
            fastFlag = 1;

        // [fast][fb+1][rl+1]: walk(0)/slow(1)/fast(2) x back/stop/front x left/stop/right.
        // map->GetAction() resolves each to the player's current stance family, so
        // walking preserves crouch/prone without the weapon-transition logic below.
#ifdef __GNUC__
        static const int selectActionUI[3][3][3] =
#else
        static const ManAction selectActionUI[3][3][3] =
#endif
            {
                {
        ...
        ManAction selAction = (ManAction)selectActionUI[fastFlag][fbFlag + 1][rlFlag + 1];
        if (selAction == ManActStop && fabs(_turnWanted) > 0.5f)
            selAction = _turnWanted < 0 ? ManActTurnL : ManActTurnR;

        MoveId moveWanted = mapUI ? mapUI->GetAction(selAction) : GetDefaultMove();
    ...
    else if (!forceAction)
    {
        ...
        // Forced walk: stamina fully depleted, OR carrying too much (>= 50 kg). Use the
        // SLOW row (1) - the same move family as normal W movement - rather than the
        // dedicated WalkF; the speed cap (SoldierOldSim.cpp) then slows it to a walk.
        if ((_sprintExhausted || _tooHeavyToRun) && !IsDown())
        {
            fastFlag = 1;

            // If the player is actively trying to sprint while over-encumbered, tell
            // them with a "Taking ammo"-style title, rate-limited so it doesn't spam.
            if (_tooHeavyToRun && turbo && (float)(Glob.time - _encumberHintTime) > 3.0f)
            {
                _encumberHintTime = Glob.time;
                if (GWorld->CameraOn() == this)
                    // TitPlainDown -> the lower on-screen spot the vanilla action text
                    // (e.g. "Refueling") uses. Effect time is divided by `speed`, so a
                    // SMALL value makes it near-instant.
                    GWorld->SetTitleEffect(
                        CreateTitleEffect(TitPlainDown, RString("You are too encumbered to run"), 0.15f));
            }
        }
...
// True only if `patient` is the single NEAREST alive, wounded person to `healer`
// within reach - so a battlefield full of casualties yields exactly one "Heal
// soldier" action, on the closest one. Ties (equal distance) are broken by pointer
// so exactly one candidate ever wins.
static bool IsNearestWoundedPerson(Person* healer, Man* patient, float reach)
{
    if (!healer || !patient || !GLandscape)
        return healer && patient;   // no landscape to scan: fall back to allowing it
    const Vector3 hp = healer->Position();
    const float pd2 = hp.DistanceXZ2(patient->Position());
    const float reach2 = reach * reach;
    if (pd2 > reach2)
        return false;
    int xMin, xMax, zMin, zMax;
    ObjRadiusRectangle(xMin, xMax, zMin, zMax, hp, hp, reach);
    for (int x = xMin; x <= xMax; x++)
    {
        for (int z = zMin; z <= zMax; z++)
        {
            ...
            for (int i = 0; i < list.Size(); i++)
            {
                Man* m = dyn_cast<Man>(list[i]);
                if (!m || m == healer || m == patient)
                    continue;
                if (m->IsDead() || m->GetTotalDammage() <= 0.0f)
                    continue;
                const float d2 = hp.DistanceXZ2(m->Position());
                if (d2 > reach2)
                    continue;
                // A strictly closer casualty (or an equidistant one that wins the
                // pointer tie-break) means `patient` is not the one to offer.
                if (d2 < pd2 || (d2 == pd2 && (const void*)m < (const void*)patient))
                    return false;
            }
        }
    }
...
void Man::GetActions(UIActions& actions, AIUnit* unit, bool now)
{
    ...
    // Heal another wounded person with your own first-aid kit. Offered only when you
    // are on foot, they are alive and hurt (< 100% health), you carry a kit with uses
    // left, they are within ~2 m, AND they are the NEAREST such casualty to you. The
    // last two matter because GetActions is called for every visible target: without
    // them a battlefield of wounded would stack up many identical "Heal soldier"
    // entries. The heal + kit consumption run in EntityAI::PerformAction (ATHeal +
    // kMedkitHealParam); the animations in Man::StartActionProcessing.
    const float kHealReach = 2.0f;
    if (free && !_isDead && veh && veh != this && GetTotalDammage() > 0.0f &&
        veh->Position().DistanceXZ2(Position()) <= kHealReach * kHealReach)
    {
        bool healerHasKit = false;
        for (int i = 0; i < veh->NMagazines() && !healerHasKit; i++)
        {
            const Magazine* m = veh->GetMagazine(i);
            if (m && m->_type && m->_ammo > 0 &&
                IsFirstAidKitMagazine(m->_type->GetName()))
                healerHasKit = true;
        }
        if (healerHasKit && IsNearestWoundedPerson(veh, this, kHealReach))
            actions.Add(ATHeal, this, 0.5f, kMedkitHealParam, true, true, 0);
    }
