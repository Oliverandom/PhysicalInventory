      ...
      _turnWanted(0), _head(Type()->_head, _shape)
{
    ...
    // First-aid-kit distribution: every West soldier carries one US kit; every East
    // and Resistance soldier one AI-2 kit; medics (attendants) carry three. Kits are
    // carry-only magazines injected at startup by RegisterMedkits (GameState.cpp).
    // Only on a full spawn (not previews/proxies); civilians get none.
    if (fullCreate)
    {
        const char* kit = nullptr;
        switch (Type()->GetTypicalSide())
        {
            case TWest:     kit = "US_FirstAidKit";  break;
            case TEast:     kit = "AI2_FirstAidKit"; break;
            case TGuerrila: kit = "AI2_FirstAidKit"; break;
            default: break;
        }
        if (kit)
        {
            int copies = Type()->IsAttendant() ? 3 : 1;
            for (int i = 0; i < copies; i++)
                AddMagazine(RStringB(kit), true, 0);
        }
    }
}
