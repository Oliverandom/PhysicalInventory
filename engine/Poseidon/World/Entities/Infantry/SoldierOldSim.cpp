...
#include <Poseidon/World/Entities/Weapons/ItemMass.hpp>
#include <Poseidon/Game/TitEffects.hpp>   // CreateTitleEffect / TitPlain (Refueling/Syphoning)
#include <Poseidon/Input/InputSubsystem.hpp>
#include <limits.h>
#include <stdio.h>
#include <cmath>
...
// Debug-menu tunable (defined in UI/Inventory/InventoryModel.cpp): floor for the
// weight mouse-sensitivity falloff (0.01 = frozen at max weight, 1.0 = no falloff).
float InventoryMouseSensFloor();
bool  InventoryInfiniteStamina();
void  CutScene(const char *name);   // Transport.cpp: vanilla title+sound feedback
...
bool Man::SimulateAnimations(float& turn, float& moveX, float& moveZ, float deltaT, SimulationImportance prec)
{
    ...
    // Speed ceiling from tiredness AND carried weight (player only; AI keep the vanilla
    // linear tired falloff). Sprinting eases toward a jog as the stamina bar empties;
    // carried load lowers the sprint ceiling from 20 kg and the walk ceiling from 35 kg.
    float maxRunTired;
    if (QIsManual())
    {
        const float stamina = 1.0f - _tired;
        const float d = 1.0f - stamina;
        const float drop = d * d * d;                                    // flat near full, steep near empty
        float staminaCap = FastRunSpeed - (FastRunSpeed - RunSpeed) * drop;   // 7.2 -> 4.9 by empty

        const float wt = ItemMass::TotalCarried(this);

        float wSprintCap = FastRunSpeed;                 // weight cap on sprinting
        if (wt > 20.0f)
        {
            float t = (wt - 20.0f) / 30.0f;              // 0..1 over 20..50 kg
            if (t > 1.0f) t = 1.0f;
            wSprintCap = FastRunSpeed - (FastRunSpeed - RunSpeed) * t;   // 7.2 -> 4.9
        }

        // A run intent takes the tighter of the stamina/weight sprint caps; a walk
        // intent is left at the normal walk speed (no red walk falloff for now).
        const float sprintCap = (staminaCap < wSprintCap) ? staminaCap : wSprintCap;
        maxRunTired = (fabs(speedZ) > 2.5f) ? sprintCap : WalkSpeed;

        // Forced walk (empty stamina OR >= 50 kg): play the SlowF move at 75% speed
        // (adjustTimeCoef 0.75) rather than crawling it into slow-motion.
        if (_sprintExhausted || _tooHeavyToRun)
            maxRunTired = 0.75f * fabs(speedZ);
    }
    else
    {
        maxRunTired = _tired * TiredRunSpeed + (1 - _tired) * FastRunSpeed;
    }

    float adjustTimeCoef = 1.0f;
    if (!QIsManual())
    {
        // AI self-heal: a wounded AI soldier uses its own first-aid kit once it
        // drops below 80% health (NeedsAmbulance >= 0.20), but only in a lull (no
        // current fire target) so it won't stop to bandage mid-firefight. Throttled
        // to a few seconds; each kit use is a full heal, so one use per wounded
        // episode restores it.
        if (!IsDead() && Brain() && !Brain()->IsPlayer() && Brain()->IsFreeSoldier() &&
            (float)(Glob.time - _selfHealTime) > 3.0f)
        {
            _selfHealTime = Glob.time;
            if (NeedsAmbulance() >= 0.20f && !GetFireTarget())
            {
                // Push the next check well past the medic animation so the heal
                // lands (and NeedsAmbulance drops) before we could re-trigger and
                // waste a second use on the same wound.
                if (TrySelfHealWithMedkit())
                    _selfHealTime = Glob.time + 6.0f;
            }
        }
    ...
    {
        // Limit movement by the stamina/weight ceiling. Unlike vanilla we do NOT clamp
        // back UP to the move's minSpd here, so a heavy load can actually slow the walk
        // (and the exhausted / over-encumbered forced walk) below its normal minimum.
        if (fabs(speedZ) > 1e-6 && speedZ > maxRunTired)
        {
            adjustTimeCoef = maxRunTired / speedZ;
            if (adjustTimeCoef > maxSpd) adjustTimeCoef = maxSpd;
            if (adjustTimeCoef < 0.15f)  adjustTimeCoef = 0.15f;   // absolute floor
        }
    }
...
void Man::Simulate(float deltaT, SimulationImportance prec)
{
    ...
    if (!CheckPredictionFrozen())
    {
        ...
        if (QIsManual())
        {
            // --- First-aid-kit self-heal (cancellable) -----------------------------
            // The medic animation is playing; heal + spend a use only when it finishes.
            // Pressing any movement key (WASD) cancels it so an accidental use costs
            // nothing.
            if (_medkitHealPending)
            {
                InputSubsystem& in = InputSubsystem::Instance();
                const bool moving = fabs(in.GetMoveForward()) + fabs(in.GetMoveBack()) +
                                    fabs(in.GetMoveLeft()) + fabs(in.GetMoveRight()) +
                                    fabs(in.GetMoveFastForward()) + fabs(in.GetMoveSlowForward())
                                    > 0.01f;
                if (moving)
                {
                    _medkitHealPending = false;   // cancelled - no kit used
                }
                else if ((float)(Glob.time - _medkitHealDone) >= 0.0f)
                {
                    Repair(1.0f);                 // full heal
                    ConsumeOneMedkitUse();        // spend one use
                    _medkitHealPending = false;
                }
            }

            // --- Jerry-can fuel transfer -------------------------------------------
            // Move 1 litre every 0.35 s (20 L in 7 s) between a nearby vehicle/station
            // and the player's jerry can, showing "Refueling"/"Syphoning". Cancels if
            // the can/target is gone, the source/destination runs out, or you walk off.
            if (_fuelDir != 0)
            {
                EntityAI* veh = _fuelTarget;
                Magazine* can = FindJerryCan(_fuelDir < 0);   // syphon needs room; refuel needs fuel
                const bool station = veh && veh->GetType() &&
                                     IsFuelStationClass(veh->GetType()->GetName());
                bool ok = veh && can;
                if (ok)
                {
                    const float rr = veh->GetRadius() + 4.0f;
                    if (Position().DistanceXZ2(veh->Position()) > rr * rr)
                        ok = false;
                }
                if (!ok)
                {
                ...
                else
                {
                    ...
                    if ((float)(Glob.time - _fuelStepTime) >= 0.35f)
                    {
                        bool moved = false;
                        if (_fuelDir > 0)   // pour 1 L from the can into the vehicle
                        {
                            if (can->_ammo > 0 && veh->GetFuel() < cap - 0.001f)
                            {
                                can->_ammo -= 1;
                                veh->Refuel(1.0f);
                                moved = true;
                        ...
                        else                // syphon 1 L into the can
                        {
                            if (can->_ammo < can->_type->_maxAmmo &&
                                (station || veh->GetFuel() > 0.001f))
                            {
                                can->_ammo += 1;
                                if (!station)
                                    veh->Refuel(-1.0f);
                        ...
                        _fuelStepTime = Glob.time;
                        if (!moved)
                            _fuelDir = 0;   // source empty / destination full
                    }
                    // Same on-screen feedback as vanilla refuelling: the "Refueling" /
                    // "Syphoning" cut-scene (lower title, flicker and pouring sound).
                    // CutScene prolongs a matching title each frame and plays the sound
                    // once, so it behaves exactly like the stock fuel-truck text.
                    if (_fuelDir != 0 && GWorld->CameraOn() == this)
                        CutScene(_fuelDir > 0 ? "JerryRefuel" : "JerrySyphon");
                }
            }

            // Player stamina schedule: a full bar lasts ~2 km of running (drain by the
            // distance actually covered), and recharges over 30 s walking / 25 s
            // standing still / 20 s crouched or prone. `intent` is the INTENDED move
            // speed (sprint/jog/walk) so it still reads as "running" even after the
            // curve caps the actual speed toward a walk; `actual` mirrors that same cap.
            const int mpos = GetActUpDegree();
            const bool prone  = (mpos == ManPosLying || mpos == ManPosLyingNoWeapon ||
                                 mpos == ManPosHandGunLying || mpos == ManPosBinocLying);
            const bool crouch = (mpos == ManPosCrouch || mpos == ManPosHandGunCrouch);

            float sZ, mnSpd, mxSpd;
            GetRelSpeedRange(sZ, mnSpd, mxSpd);
            const float intent = fabs(sZ);                  // wanted move speed

            // Per-side conditioning: soldiers (West/East) get the full 2 km sprint and
            // baseline recovery; civilians are the least fit (800 m, 1.75x recovery
            // time) and resistance fighters in between (1200 m, 1.5x).
            float sprintDist = 2000.0f;
            float recoverMult = 1.0f;
            if (AIUnit* u = Brain())
                if (AIGroup* g = u->GetGroup())
                    if (AICenter* c = g->GetCenter())
                    {
                        const int side = c->GetSide();
                        if (side == TCivilian)      { sprintDist = 800.0f;  recoverMult = 1.75f; }
                        else if (side == TGuerrila) { sprintDist = 1200.0f; recoverMult = 1.5f;  }
                    }

            // Carried weight: past 20 kg each kg speeds stamina burn (+3%/kg), and at
            // 50 kg the soldier is too encumbered to run at all (forced walk + popup,
            // handled in the move-selection code via _tooHeavyToRun).
            const float wt = ItemMass::TotalCarried(this);
            // Sprint stamina burn: gentle through the yellow band, then an exponential
            // collapse through amber so a full bar empties in only ~1 s around 47 kg.
            // (Base bar is a few hundred seconds; time-to-empty = base / mult.)
            //   20 kg -> x1   (unchanged, several hundred s)
            //   35 kg -> x1.2 (a few dozen seconds less)
            //   40 kg -> x11.6 (~24 s)     45 kg -> x112 (~2.5 s)     47 kg -> x278 (~1 s)
            float weightDrainMult = 1.0f;
            if (wt > 35.0f)      weightDrainMult = 1.2f * expf(0.4538f * (wt - 35.0f));
            else if (wt > 20.0f) weightDrainMult = 1.0f + (wt - 20.0f) * 0.0133f;
            _tooHeavyToRun = (wt >= 50.0f);

            // Backpack inertia: mouse look sensitivity eases off with weight, and the
            // falloff STEEPENS band by band - gentle in amber (20-35), stronger in dark
            // amber (35-50), strongest in red (50-60). Reaches ~100/80/50/25% at the
            // 20/35/50/60 kg marks, floored at 25%.
            // Aggressive base falloff (1.00 at 20 kg -> 0.01 at 60 kg).
            float base = 1.0f;
            if (wt > 50.0f)      base = 0.10f - (wt - 50.0f) * 0.009f;   // red:        0.10 -> 0.01
            else if (wt > 35.0f) base = 0.50f - (wt - 35.0f) * 0.0267f;  // dark amber: 0.50 -> 0.10
            else if (wt > 20.0f) base = 1.00f - (wt - 20.0f) * 0.0333f;  // amber:      1.00 -> 0.50
            if (base < 0.01f) base = 0.01f;
            // The debug slider sets the FLOOR (min look scale) at max weight: 0.01 =
            // frozen (full falloff), 1.0 = disabled. Remap the base curve onto it.
            const float floorv = InventoryMouseSensFloor();
            const float severity = (1.0f - base) / 0.99f;               // 0 (light) .. 1 (max)
            float aimScale = 1.0f - severity * (1.0f - floorv);
            InputSubsystem::Instance().SetPlayerLookScale(aimScale);

            // Only a SPRINT costs stamina - walking and normal running (jog) are free.
            // Threshold sits between jog and sprint so only the fast-run move qualifies.
            if (InventoryInfiniteStamina())
            {
                _tired = 0.0f;   // debug: never fatigue
            }
            else if (!prone && !crouch && intent > (RunSpeed + FastRunSpeed) * 0.5f)
            {
                // Fixed, continuous drain while sprinting - keyed to the full sprint
                // speed so a full bar empties over `sprintDist` metres of sprinting,
                // and does NOT falter as the speed curve slows the soldier near empty.
                _tired += (FastRunSpeed * deltaT) / sprintDist * weightDrainMult;
            }
            else
            {
                float rechargeSecs = 25.0f;                       // standing still
                if (prone || crouch)        rechargeSecs = 20.0f; // crawling / crouching
                else if (intent > 0.3f)     rechargeSecs = 30.0f; // walking
                _tired -= deltaT / (rechargeSecs * recoverMult);  // slower for civ/resistance
            }
        }
        else
        {
            // "bad" soldiers are tired much faster (vanilla AI fatigue).
            _tired += duty * (1.0f / 20) * personality * deltaT;
        }
        saturate(_tired, 0, 1);

        // Player: once the bar empties, lock out sprint (forced walk) until it has
        // recovered ~25%, so the walk/sprint switch doesn't flicker at the boundary.
        if (QIsManual())
        {
            // Forced walk at a fully empty bar; a small 2% recovery band re-enables
            // sprint (just enough to stop the walk/sprint flicker at the boundary).
            if (_tired >= 1.0f)       _sprintExhausted = true;
            else if (_tired <= 0.98f) _sprintExhausted = false;   // recovered ~2% -> sprint ok
        }
