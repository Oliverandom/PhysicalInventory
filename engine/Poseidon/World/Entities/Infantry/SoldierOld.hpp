...
class Man: public Person
{
	...
	float _tired;
	bool _sprintExhausted = false;   //!< player: bar hit empty; forced to walk until recovered
	bool _tooHeavyToRun = false;     //!< player: carried weight >= 50 kg; forced to walk
	Foundation::Time _encumberHintTime;   //!< last time the "too encumbered" popup showed
	Foundation::Time _selfHealTime;       //!< AI: last time the self-heal medkit check ran
	bool _medkitHealPending = false;      //!< self-heal medic animation playing (heals when it finishes)
	Foundation::Time _medkitHealDone;     //!< time the self-heal completes, unless cancelled by moving
	int _fuelDir = 0;                     //!< jerry can: 0 idle, +1 refuel vehicle, -1 syphon into can
	Foundation::Time _fuelStepTime;       //!< last 1-litre fuel transfer step
	Foundation::Time _fuelHintTime;       //!< last "Refueling/Syphoning" on-screen title refresh
	LLink<EntityAI> _fuelTarget;          //!< vehicle / gas station being fuelled or syphoned
	...
	int GetActUpDegree() const;  // actual current stance/pose (ManPos*)
	float GetTired() const { return _tired; }  // fatigue 0=fresh..1=exhausted (stamina = 1-this)

	//! AI self-heal: if this soldier carries a first-aid kit with uses left, trigger a
	//! full self-heal via the same ATUseMagazine path the player uses. Returns true if
	//! a kit was used. Caller decides WHEN (wound level, combat lull, throttle).
	bool TrySelfHealWithMedkit();

	//! Consume one use of the first first-aid kit this soldier carries (removing the kit
	//! at zero uses). Used when healing ANOTHER person with your medkit. Returns true if
	//! a use was consumed.
	bool ConsumeOneMedkitUse();

	//! Find the player's first jerry can: one with room to fill (needRoom=true, for
	//! syphoning in) or one holding fuel (needRoom=false, for pouring out). Null if none.
	Magazine* FindJerryCan(bool needRoom);

	//! Active jerry-can fuel transfer: 0 = none, +1 = refuelling a vehicle, -1 = syphoning.
	int FuelDir() const { return _fuelDir; }
	//! The vehicle / gas station currently being fuelled or syphoned (null if idle).
	EntityAI* FuelTarget() const { return _fuelTarget; }
	...
	float GetBodyHit() const {return GetHit(Type()->_bodyHit);}   // torso damage (lethal)
	float GetHeadHit() const {return GetHit(Type()->_headHit);}   // head damage (lethal)
