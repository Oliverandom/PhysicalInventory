...
void Man::StartActionProcessing(const UIAction& action, AIUnit* unit)
{
    switch (action.type)
    {
        case ATHeal:
        {
            if (action.param == kMedkitHealParam)
            {
                // Heal another person with a first-aid kit: 'this' is the healer, who
                // plays the medic (administering) animation; the patient plays the
                // being-treated animation. Apply the full heal and consume one kit use
                // directly here - relying on the action-completion callback proved
                // unreliable for this cross-unit action (it never fired, so the target
                // was never healed and no use was spent).
                Person* patient = dyn_cast<Person, EntityAI>(action.target);
                if (patient)
                {
                    patient->PlayAction(ManActTreated);
                    patient->PlayAction(ManActUp);
                    patient->Repair(1.0f);          // full heal of the patient
                    // Report the heal the same way the base game does once a medic
                    // finishes healing a unit (AISubgroupFSMSupply.inc ReportStatus for
                    // Command::Heal): reset the patient's REPORTED health to normal so
                    // the command bar and the group's health AI see them as recovered
                    // (the reported state is otherwise sticky until the unit re-reports).
                    if (AIUnit* pu = patient->Brain())
                        if (AIGroup* pg = pu->GetGroup())
                            pg->SetHealthStateReported(pu, AIUnit::RSNormal);
                }
                PlayAction(ManActMedic);
                PlayAction(ManActUp);
                ConsumeOneMedkitUse();              // spend one use of the healer's kit
                break;
            }
            Heal(this, action);
            break;
        ...
        case ATUseMagazine:
        {
            const Magazine* mag = FindMagazine(action.param, action.param2);
            if (mag && IsFirstAidKitMagazine(mag->_type->GetName()))
            {
                // First-aid kit self-heal: play the medic animation, but DON'T heal or
                // spend a use yet - defer both to the soldier sim so moving (WASD) can
                // cancel it (e.g. used by accident) before it completes.
                PlayAction(ManActMedic);
                PlayAction(ManActUp);
                _medkitHealPending = true;
                _medkitHealDone = Glob.time + 2.5f;   // ~medic-animation length
            }
            else
            {
                Ref<ActionContextUIAction> context = new ActionContextUIAction(action);
        ...
        case ATRefuel:
        {
            // Jerry-can fuelling of a nearby vehicle / gas station: start a timed
            // transfer (run 1 litre at a time in the soldier sim). Vanilla refuel
            // (param 0) falls through to the normal put-down handling below.
            if (action.param == kJerryRefuelParam || action.param == kJerrySyphonParam)
            {
                _fuelTarget = (EntityAI *)action.target;
                _fuelDir = (action.param == kJerryRefuelParam) ? +1 : -1;
                _fuelStepTime = Glob.time;
                _fuelHintTime = Glob.time - 10.0f;   // show the title immediately
                break;
            }
            Ref<ActionContextUIAction> context = new ActionContextUIAction(action);
            PlayAction(ManActPutDown, context);
...
bool Man::TrySelfHealWithMedkit()
{
    for (int m = 0; m < NMagazines(); m++)
    {
        Magazine* mag = GetMagazine(m);
        if (!mag || !mag->_type || mag->_ammo <= 0)
            continue;
        if (!IsFirstAidKitMagazine(mag->_type->GetName()))
            continue;
        // Route the same ATUseMagazine action the player uses, so the medic
        // animation, full heal and use-depletion all run identically for AI.
        UIAction act;
        ...
        act.hideOnUse = true;
        StartActionProcessing(act, Brain());
        return true;
    }
    return false;
}

bool Man::ConsumeOneMedkitUse()
{
    for (int m = 0; m < NMagazines(); m++)
    {
        Magazine* mag = GetMagazine(m);
        ...
        if (mag->_ammo <= 0)
        {
            Ref<Magazine> dead = mag;
            for (int j = 0; j < _magazineSlots.Size(); j++)
            {
                if (_magazineSlots[j]._magazine == dead)
...
Magazine* Man::FindJerryCan(bool needRoom)
{
    for (int m = 0; m < NMagazines(); m++)
    {
        Magazine* mag = GetMagazine(m);
        if (!mag || !mag->_type || !IsJerryCanMagazine(mag->_type->GetName()))
            continue;
        if (needRoom) { if (mag->_ammo < mag->_type->_maxAmmo) return mag; }
        else          { if (mag->_ammo > 0)                    return mag; }
    }
    return nullptr;
}
