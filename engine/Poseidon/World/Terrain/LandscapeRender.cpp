...
{
    // Guard each celestial object independently. These are created only when
    // their shape loads (Landscape.cpp), so a scene missing e.g. the moon or
    // stars must still draw the sky dome and sun. Bailing out of the whole
    // function on any null left the backdrop unpainted -> blown-out scene.
