...
void Landscape::Init()
{
    ...
    // CfgLandscapeSky lives only in the optional Remaster catalog (remaster.bin,
    // absent on a bare launch without -mod=Remastered). When it is missing,
    // `sky >> slot >> "model"` yields an empty string and every sky object would
    // end up null -> no sky drawn / crash. Fall back to the stock model paths,
    // which ship in the base Dta/Data3D.pbo, so the sky still loads. These match
    // the defaults the original engine baked in.
    auto skyModel = [&sky](const char* slot, const char* fallback) -> RStringB
    {
        RStringB name = sky >> slot >> "model";
        if (name.GetLength() == 0)
        {
            LOG_WARN(World, "CfgLandscapeSky slot '{}' unavailable; using default '{}'", slot, fallback);
            return RStringB(fallback);
        }
        return name;
    };
    RStringB skyName = skyModel("sky", "data3D\\obloha.p3d");
    RStringB starsName = skyModel("stars", "data3D\\stars.p3d");
    RStringB horizontName = skyModel("horizont", "data3D\\horizont.p3d");
    RStringB sunHaloName = skyModel("sunHalo", "data3D\\sunHalo.p3d");
    RStringB sunName = skyModel("sun", "data3D\\sun.p3d");
    RStringB moonName = skyModel("moon", "data3D\\moon.p3d");
