...
bool World::InitVehicles(GameMode gameMode, ArcadeTemplate& t)
{
    ...
    // NOTE: eager inventory-icon preload removed - it ran on the first InitVehicles
    // only (static guard), which made the first editor Preview bounce back before
    // loading. Icons now load on demand; the offline-baked crop rects (_crops.txt)
    // already removed the expensive per-icon pixel scan, so on-demand is cheap.
