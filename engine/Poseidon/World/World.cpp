...
void World::Simulate(float deltaT, bool& enableDraw)
{
...

    {
        ...
        else if (cameraVehicle)
        {
            ...
            // Also require the in-game UI to be enabled: a non-pausing modal
            // (e.g. the inventory, which keeps the world simulating but sets
            // _enableUI=false) must freeze the camera/look, not just body turn.
            // Normal play and the map keep IsUIEnabled() true, so this only
            // gates the UI-disabled overlay case.
            if (!HasOptions() && IsUIEnabled())
            {
